...
### Changed — Dynamic Live Exchange Rate for PayPal Checkout

Eliminates the financial risk of static exchange rates by wiring `CurrencyService`
to a live USD/VND feed with a 24-hour cache and a safe static fallback.

#### `app/Services/CurrencyService`

- **New method `getLiveUsdToVndRate(): float`**
  - Calls `https://open.er-api.com/v6/latest/USD` (free, no API key) via Laravel's
    `Http` facade with a configurable timeout (default 5 s).
  - On success: stores the rate in `Cache::remember('usd_vnd_rate', 86400, ...)` for
    exactly 24 hours and returns it.
  - On any failure (timeout, bad response, no VND key): logs a `warning`, returns the
    static `.env` fallback (`CURRENCY_VND_TO_USD_RATE`) **without caching**, so the live
    fetch is automatically retried on the next request.
- **New method `forgetCachedRate(): void`** — busts the cache key for admin/Artisan use.
- **Updated `vndToUsd(int $vnd): float`** — now calls `getLiveUsdToVndRate()` instead of
  reading the config directly.
- **Updated static `convert()`** — reads `Cache::get('usd_vnd_rate')` first, falls back
  to config, keeping the static helper consistent with the live rate.

#### `config/currency.php`

| New key | Env override | Default |
|---------|-------------|---------|
| `exchange_rate_api_url` | `CURRENCY_EXCHANGE_RATE_API_URL` | `https://open.er-api.com/v6/latest/USD` |
| `exchange_rate_timeout` | `CURRENCY_EXCHANGE_RATE_TIMEOUT` | `5` (seconds) |

The existing `vnd_to_usd_rate` / `CURRENCY_VND_TO_USD_RATE` key is now documented
explicitly as the **static fallback** used only when the live API is unreachable.

#### `CheckoutController::processPayPal()`

- Removed dependency on the hardcoded `config('paypal.plans.*.amount_usd')`.
- Now fetches the live rate via `$this->currencyService->getLiveUsdToVndRate()`.
- Converts the VND cart total to USD with `round($total / $liveRate, 2)` — exactly as
  specified.
- **Locks the rate** into `Order::exchange_rate_used` at the moment the PayPal order is
  created, so the charged amount is always auditable.
- `CurrencyService` is now constructor-injected into `CheckoutController`.

#### Database

- **New migration** `2026_06_17_200000_add_exchange_rate_used_to_orders_table`
  — adds `exchange_rate_used DECIMAL(12,4) NULL` to the `orders` table.
  Nullable so PayOS (VND-only) orders are unaffected.
- **`Order` model** — `exchange_rate_used` added to `$fillable` and cast to `float`.

#### Failure / Fallback Matrix

| Scenario | Behaviour |
|----------|-----------|
| API reachable, valid VND rate | Rate cached 24 h, used for conversion |
| API reachable, missing VND key | Exception thrown → fallback, no cache |
| API timeout / HTTP error | Exception caught → fallback, no cache |
| Cache hit (within 24 h) | Cached value returned, no HTTP call made |
| `forgetCachedRate()` called | Next request re-fetches from API |

#### Also fixed

- `AppServiceProvider`: guarded `Dedoc\Scramble` registration with `class_exists()`
  so Artisan commands (including `migrate`) work when the package is not installed.
