...
const WalletManagement: React.FC = () => {
  const { t } = useTranslation();
  const {
    accounts, categories, loading, errorMsg, search, setSearch, sortKey, setSortKey,
    ...
    deleteError, displayed, totals, adjustDiff, relevantCategories, fetchAccounts,
    goToList, openAdd, openEdit, openAdjust, handleCreate, handleUpdate, handleAdjust,
    handleDelete,
  } = useWalletManagement();

  // Data loading, derived list state, navigation, and all mutations live in
  // useWalletManagement so the page remains a layout composition.

  // Create and metadata-only update are handled by useWalletManagement.

  // Adjust Balance and delete confirmation are kept in the feature hook so
  // transaction creation remains the only balance-changing path.
